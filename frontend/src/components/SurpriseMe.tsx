...
export function SurpriseMe({ events }: Props) {
  ...
  const spin = useCallback(() => {
    ...
    const targetIdx = Math.floor(Math.random() * SEGMENT_COUNT);
    const targetCat = SEGMENTS[targetIdx]!.category;
    ...
    setTimeout(() => {
      ...
      const catEvents = events.filter(e => getVenueCategory(e.venue_id) === targetCat);
      if (catEvents.length > 0) {
        const idx = Math.floor(Math.random() * catEvents.length);
        setPicked(catEvents[idx] ?? null);
      }
    }, 3000);
  }, [spinning, events, rotation]);

  return (
    <div className="rounded-2xl border border-dashed border-stone-300 bg-stone-50/50 p-6 dark:border-purple-900/40 dark:bg-[#16101e]/50">
